...
from ev_rgb_isp.datasets.basic_batch import DemosaicHybridevsBatch as Batch
...
def get_unet_image_swin(config):
    ...
    if config.TYPE == "costom":
        ...
        model = get_unet_image_swin_large(using_rggb_4_channel, in_chans)
    elif config.TYPE == "medium":
        model = get_unet_image_swin_medium(using_rggb_4_channel, in_chans)
    elif config.TYPE == "small":
        model = get_unet_image_swin_small(using_rggb_4_channel, in_chans)
    elif config.TYPE == "tiny":
        model = get_unet_image_swin_tiny(using_rggb_4_channel, in_chans)
    return model


def get_unet_image_swin_large(using_rggb_4_channel, in_chans):
    model = UNetImageSwin(
        using_rggb_4_channel=using_rggb_4_channel,
        in_chans=in_chans,
        embed_dim=96,
...
def get_unet_image_swin_medium(using_rggb_4_channel, in_chans):
    model = UNetImageSwin(
        using_rggb_4_channel=using_rggb_4_channel,
        in_chans=in_chans,
        embed_dim=96,
...
def get_unet_image_swin_small(using_rggb_4_channel, in_chans):
    model = UNetImageSwin(
        using_rggb_4_channel=using_rggb_4_channel,
        in_chans=in_chans,
        embed_dim=96,
...
def get_unet_image_swin_tiny(using_rggb_4_channel, in_chans):
    model = UNetImageSwin(
        using_rggb_4_channel=using_rggb_4_channel,
        in_chans=in_chans,
        embed_dim=96,
...
class UNetImageSwin(nn.Module):
    ...
    def forward(self, batch):
        x = batch[Batch.RAW_TENSOR]  # B, C, H, W
        position = batch[Batch.RAW_RGB_POSITION]
